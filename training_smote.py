...
def train_teacher_cnn(model, X_train, y_train, X_val, y_val, epochs=30, batch_size=64, checkpoint_dir="checkpoints", output_mode='device'):
...
def train_student_no_kd(model, X_train, y_train, X_val, y_val, epochs=30, batch_size=64, checkpoint_dir="checkpoints", output_mode='device', fold=None):
...
def train_student_kd(teacher, student, feature_model, X_train, y_train, X_val, y_val, alpha=0.7, temperature=2.0,
                     epochs=30, batch_size=64, checkpoint_dir="checkpoints", output_mode='device', loss_type='kl',
...
def train_and_evaluate_fold(X_train_val, y_train_val, X_test, y_test, feature_names, dataset_name, output_dir, checkpoint_dir,
    ...
    Args:
        X_train_val: Training and validation features (already balanced).
        y_train_val: Training and validation labels (already balanced).
    ...
    # Use a portion of X_train_val for validation (20% as in the original split)
    if fold is None:
        val_size = 0.2
        if output_mode == 'multi':
            X_train, X_val, y_train, y_val = train_test_split(
                X_train_val, y_train_val, test_size=val_size, random_state=42,
                stratify=np.column_stack(y_train_val) if y_train_val[0].ndim > 1 else y_train_val[0])
        ...
        # For cross-validation, use X_test as validation and X_train_val as training
        X_train, X_val, y_train, y_val = X_train_val, X_test, y_train_val, y_test
    ...
    teacher_model, teacher_train_metrics = train_teacher_cnn(
        teacher_model, X_train_cnn, y_train, X_val_cnn, y_val, epochs=30, batch_size=64,
