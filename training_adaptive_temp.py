...
def train_teacher_cnn(model, X_train, y_train, X_val, y_val, epochs=100, batch_size=32, checkpoint_dir="checkpoints",
                      output_mode='device'):
    ...
    if output_mode == 'multi':
        ...
        train_dataset = tf.data.Dataset.from_tensor_slices(
            (X_train, {'device': y_train_device, 'attack': y_train_attack})).batch(batch_size).prefetch(
            tf.data.AUTOTUNE)
        val_dataset = tf.data.Dataset.from_tensor_slices(
            (X_val, {'device': y_val_device, 'attack': y_val_attack})).batch(batch_size).prefetch(tf.data.AUTOTUNE)
...
def train_student_no_kd(model, X_train, y_train, X_val, y_val, epochs=100, batch_size=32, checkpoint_dir="checkpoints",
...
def train_student_kd(teacher, student, feature_model, X_train, y_train, X_val, y_val, alpha=0.5, temperature=5.0,
                     epochs=100, batch_size=32, checkpoint_dir="checkpoints", output_mode='device', loss_type='kl',
    ...
    if output_mode == 'multi':
        history.update({'device_loss': [], 'attack_loss': [], 'device_accuracy': [], 'attack_accuracy': [],
                        'val_device_loss': [], 'val_attack_loss': [], 'val_device_accuracy': [],
                        'val_attack_accuracy': []})
    ...
    for epoch in range(epochs):
            ...
            with tf.GradientTape() as tape:
                ...
                if output_mode == 'multi':
                    ...
                    acc_device = tf.reduce_mean(
                        tf.cast(tf.equal(tf.argmax(y_pred_device, axis=-1), tf.cast(y_batch_device, tf.int64)),
                                tf.float32))
                    acc_attack = tf.reduce_mean(
                        tf.cast(tf.equal(tf.argmax(y_pred_attack, axis=-1), tf.cast(y_batch_attack, tf.int64)),
                                tf.float32))
        ...
        for X_val_batch, y_val_batch, teacher_val_logits_batch in val_dataset:
            ...
            if output_mode == 'multi':
                ...
                val_loss_device = loss_fn(y_val_batch_device, y_val_pred_device, teacher_val_logits_device,
                                          **loss_kwargs)
                val_loss_attack = loss_fn(y_val_batch_attack, y_val_pred_attack, teacher_val_logits_attack,
                                          **loss_kwargs)
                val_loss_batch = 0.5 * (val_loss_device + val_loss_attack)
                val_acc_device = tf.reduce_mean(
                    tf.cast(tf.equal(tf.argmax(y_val_pred_device, axis=-1), tf.cast(y_val_batch_device, tf.int64)),
                            tf.float32))
                val_acc_attack = tf.reduce_mean(
                    tf.cast(tf.equal(tf.argmax(y_val_pred_attack, axis=-1), tf.cast(y_val_batch_attack, tf.int64)),
                            tf.float32))
        ...
        if output_mode == 'multi':
            ...
            print(
                f"Epoch {epoch + 1}, Device Loss: {epoch_device_loss / batch_count:.4f}, Attack Loss: {epoch_attack_loss / batch_count:.4f}, "
...
def train_and_evaluate_fold(X_train_val, y_train_val, X_test, y_test, feature_names, dataset_name, output_dir,
                            checkpoint_dir,
                            output_mode, le, num_classes_device, num_classes_attack, n_splits=1, fold=None,
    ...
    Args:
        ...
        distillation_method: Knowledge distillation method to use. Options:
            ...
            - 'adaptive_temperature': Online distillation with adaptive temperature adjustment.
    ...
    teacher_model, teacher_train_metrics = train_teacher_cnn(
        teacher_model, X_train_cnn, y_train, X_val_cnn, y_val, epochs=100, batch_size=32,
    ...
    mem_before = process.memory_percent()
    cpu_before = psutil.cpu_percent(interval=None)
    y_pred_proba = teacher_model.predict(X_test_cnn, batch_size=32, verbose=0)
    ...
    loss_types = ['kl', 'js']  # 'kl_js' 'uncertainty', 'dynamic',
    distillation_methods = ['feature_matching'] #'active', 'feature_matching', 'gradient_matching', 'combined', 'coreset',
                            #'generative', 'adaptive_temperature']
    for method in distillation_methods:
        for loss_type in loss_types:
            ...
            try:
                ...
                print(
                    f"Student 1D-CNN KD ({loss_type.upper()}, {method}) parameters: {student_params}, FLOPs: {student_flops}")

                # Apply distillation method
                X_train_kd = X_train_cnn
                y_train_kd = y_train
                if method == 'active':
                    X_train_kd, indices = select_active_samples(teacher_model, X_train_cnn, batch_size=32,
                                                                n_samples=10000)
                    if output_mode == 'multi':
                    ...
                    X_train_kd, y_train_kd = generate_synthetic_data(X_train_cnn, y_train, output_mode=output_mode,
                                                                     n_synthetic=10000)
                    # Use teacher to label synthetic data
                    teacher_preds = teacher_model.predict(X_train_kd, batch_size=32, verbose=0)
                    if output_mode == 'multi':
                        y_train_kd = (np.argmax(teacher_preds[0], axis=1), np.argmax(teacher_preds[1], axis=1))
                    else:
                        y_train_kd = np.argmax(teacher_preds, axis=1)
                elif method == 'adaptive_temperature':
                    # Online distillation with adaptive temperature
                    start_time = time.time()
                    process = psutil.Process()
                    mem_before = process.memory_percent()
                    cpu_before = psutil.cpu_percent(interval=None)
                    performance_history = adapt_temperature_and_track_performance(
                        teacher_model, student_model, X_train_kd, y_train_kd, X_val_cnn, y_val,
                        initial_temperature=1.0, max_epochs=100, batch_size=32
                    )
                    train_time = time.time() - start_time
                    train_time_per_sample = train_time / len(X_train_kd)
                    mem_after = process.memory_percent()
                    cpu_after = psutil.cpu_percent(interval=None)
                    train_mem_usage = (mem_before + mem_after) / 2
                    train_cpu_usage = (cpu_before + cpu_after) / 2
                    student_train_metrics = {
                        'train_time': train_time,
                        'train_time_per_sample': train_time_per_sample,
                        'train_mem_usage': train_mem_usage,
                        'train_cpu_usage': train_cpu_usage
                    ...
                    import json
                    with open(
                            checkpoint_dir / f"performance_history_{loss_type}_{method}_fold_{fold if fold else 'None'}.json",
                            'w') as f:
                    ...
                    student_model, student_train_metrics = train_student_kd(
                        ...
                        alpha=0.7, temperature=2.0, epochs=100, batch_size=32, checkpoint_dir=checkpoint_dir,
                ...
                mem_before = process.memory_percent()
                cpu_before = psutil.cpu_percent(interval=None)
                y_pred_proba = student_model.predict(X_test_cnn, batch_size=32, verbose=0)
                test_time = time.time() - start_time
                test_time_per_sample = test_time / len(X_test)
                mem_after = process.memory_percent()
                cpu_after = psutil.cpu_percent(interval=None)
                ...
                if output_mode == 'multi':
                    ...
                    y_pred = np.column_stack((y_pred_attack, y_pred_device))
                    print(
                        f"Predicted device type classes (Student 1D-CNN KD {loss_type.upper()} {method}): {np.unique(y_pred_device)}")
                    print(
                        f"Predicted attack category classes (Student 1D-CNN KD {loss_type.upper()} {method}): {np.unique(y_pred_attack)}")
                    metrics_device = evaluate_model(
                        y_test_device, y_pred_device, le_device,
                        f"Device Type (Student 1D-CNN KD {loss_type.upper()} {method})",
                        y_pred_device_proba, y_test, y_pred, lb_attack, lb_device, checkpoint_dir, fold, output_mode)
                    metrics_attack = evaluate_model(
                        y_test_attack, y_pred_attack, le_attack,
                        f"Attack Category (Student 1D-CNN KD {loss_type.upper()} {method})",
                        y_pred_attack_proba, y_test, y_pred, lb_attack, lb_device, checkpoint_dir, fold, output_mode)
                    save_predictions(
                        y_test_device, y_pred_device, y_pred_device_proba,
                        y_test_attack, y_pred_attack, y_pred_attack_proba,
                        le_device, le_attack, dataset_name, f"Student_1D-CNN_KD_{loss_type.upper()}_{method}",
                        output_dir, output_mode, fold)
                    results.append({
                        'Dataset': dataset_name,
                    ...
                    y_pred = np.argmax(y_pred_proba, axis=1)
                    print(
                        f"Predicted {target_name} classes (Student 1D-CNN KD {loss_type.upper()} {method}): {np.unique(y_pred)}")
                    metrics = evaluate_model(
                ...
                if output_mode != 'multi':
                    try:
                        ...
                        print(
                            f"SHAP computation failed for student model KD {loss_type.upper()} {method}: {e}. Continuing with pipeline.")
            except Exception as e:
    ...
    try:
        ...
        student_no_kd, student_no_kd_train_metrics = train_student_no_kd(
            ...
            epochs=100, batch_size=32, checkpoint_dir=checkpoint_dir, output_mode=output_mode, fold=fold)

        start_time = time.time()
        process = psutil.Process()
        mem_before = process.memory_percent()
        cpu_before = psutil.cpu_percent(interval=None)
        y_pred_proba_no_kd = student_no_kd.predict(X_test_cnn, batch_size=32, verbose=0)
        ...
        if output_mode == 'multi':
            ...
            metrics_device_no_kd = evaluate_model(
                ...
                y_pred_device_proba_no_kd, y_test, y_pred_no_kd, lb_attack, lb_device, checkpoint_dir, fold,
                output_mode)
            metrics_attack_no_kd = evaluate_model(
                y_test_attack, y_pred_attack_no_kd, le_attack, f"Attack Category (Student 1D-CNN No KD)",
                y_pred_attack_proba_no_kd, y_test, y_pred_no_kd, lb_attack, lb_device, checkpoint_dir, fold,
                output_mode)
            save_predictions(
...
def adapt_temperature_and_track_performance(teacher_model, student_model, X_train, y_train, X_val, y_val,
                                            initial_temperature=1.0, max_epochs=100, batch_size=32):
    """
    Adaptively adjust the temperature parameter and track model performance during training.
    ...
    Args:
        ...
        initial_temperature: Starting temperature value (default: 1.0).
        max_epochs: Maximum number of training epochs (default: 50).
        batch_size: Batch size for training (default: 32).

    Returns:
        performance_history: Dictionary containing temperature and performance metrics per epoch.
    """
    ...
    for epoch in range(max_epochs):
        print(f"Epoch {epoch + 1}/{max_epochs}, Temperature: {current_temperature:.2f}")
        ...
        for X_batch, y_batch in train_dataset:
            with tf.GradientTape() as tape:
                ...
                if isinstance(teacher_logits, list):
                    teacher_logits = teacher_logits[0]  # Use first output (device)
                teacher_probs = tf.nn.softmax(teacher_logits / current_temperature)

                student_logits = student_model(X_batch, training=True)
                if isinstance(student_logits, list):
                    student_logits = student_logits[0]  # Use first output (device)
                student_loss = loss_fn(y_batch, student_logits) + \
                               tf.reduce_mean(tf.keras.losses.KLDivergence()(teacher_probs, tf.nn.softmax(
                                   student_logits / current_temperature)))

            gradients = tape.gradient(student_loss, student_model.trainable_variables)
            optimizer.apply_gradients(zip(gradients, student_model.trainable_variables))

            train_loss += student_loss.numpy() * len(X_batch)
            train_correct += np.sum(np.argmax(student_logits.numpy(), axis=1) == y_batch.numpy())
        ...
        for X_val_batch, y_val_batch in val_dataset:
            ...
            if isinstance(val_logits, list):
                val_logits = val_logits[0]  # Use first output
            val_loss_batch = loss_fn(y_val_batch, val_logits).numpy()
            val_correct += np.sum(np.argmax(val_logits.numpy(), axis=1) == y_val_batch.numpy())
            val_samples += len(y_val_batch)
            val_loss += val_loss_batch * len(X_val_batch)

        val_loss /= val_samples
        val_accuracy = val_correct / val_samples

        performance_history['temperature'].append(current_temperature)
        performance_history['train_loss'].append(train_loss)
        performance_history['val_loss'].append(val_loss)
        performance_history['train_accuracy'].append(train_accuracy)
        performance_history['val_accuracy'].append(val_accuracy)

        if epoch > 0:
            if val_loss > performance_history['val_loss'][-2] + 0.01:
                current_temperature = min(current_temperature + 0.1, 5.0)
                print(f"Increasing temperature to {current_temperature:.2f} due to rising val loss")
            elif val_accuracy < performance_history['val_accuracy'][-2] - 0.01:
                current_temperature = max(current_temperature - 0.1, 0.1)
                print(f"Decreasing temperature to {current_temperature:.2f} due to dropping val accuracy")
