...
from utils import count_parameters, ensure_directory
...
def train_teacher_cnn(model, X_train, y_train, X_val, y_val, epochs=50, batch_size=32, checkpoint_dir="checkpoints", output_mode='device'):
...
def train_student_no_kd(model, X_train, y_train, X_val, y_val, epochs=50, batch_size=32, checkpoint_dir="checkpoints", output_mode='device', fold=None):
...
def train_student_kd(teacher, student, feature_model, X_train, y_train, X_val, y_val, alpha=0.7, temperature=2.0,
                     epochs=50, batch_size=32, checkpoint_dir="checkpoints", output_mode='device', loss_type='kl',
...
def train_and_evaluate_fold(X_train_val, y_train_val, X_test, y_test, feature_names, dataset_name, output_dir, checkpoint_dir,
    ...
    print(f"Teacher 1D-CNN parameters: {teacher_params}")
    teacher_model, teacher_train_metrics = train_teacher_cnn(
        teacher_model, X_train_cnn, y_train, X_val_cnn, y_val, epochs=50, batch_size=32,
    ...
    # loss_types = ['kl', 'js', 'kl_js', 'uncertainty', 'dynamic', 'cosine'] 'kl', 'js', 'kl_js'
    loss_types = ['uncertainty', 'dynamic', 'cosine', 'kl', 'js', 'kl_js']
    # distillation_methods = ['standard']
    distillation_methods = ['feature_matching',
                            'gradient_matching', 'combined', 'standard'] #'coreset', 'active',  'generative'
    for method in distillation_methods:
        for loss_type in loss_types:
            print(f"\nTraining Student 1D-CNN with KD ({loss_type.upper()}, {method}) ({output_mode})...")
            try:
                student_model, feature_model = build_1dcnn_small((X_train.shape[1], 1), num_classes_device, num_classes_attack)
                student_params = count_parameters(student_model)
                print(f"Student 1D-CNN KD ({loss_type.upper()}, {method}) parameters: {student_params}")

                # Apply distillation method
                X_train_kd = X_train_cnn
                y_train_kd = y_train
                if method == 'active':
                    X_train_kd, indices = select_active_samples(teacher_model, X_train_cnn, batch_size=32, n_samples=10000)
                    if output_mode == 'multi':
                ...
                student_model, student_train_metrics = train_student_kd(
                    ...
                    alpha=0.7, temperature=2.0, epochs=50, batch_size=32, checkpoint_dir=checkpoint_dir,
    ...
    try:
        ...
        print(f"Student 1D-CNN (No KD) parameters: {student_no_kd_params}")
        student_no_kd, student_no_kd_train_metrics = train_student_no_kd(
            student_no_kd, X_train_cnn, y_train, X_val_cnn, y_val,
            epochs=50, batch_size=32, checkpoint_dir=checkpoint_dir, output_mode=output_mode, fold=fold)
